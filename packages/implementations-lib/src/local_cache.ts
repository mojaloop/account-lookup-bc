...
export interface ILocalCache {
    get(...keys: (string|undefined|null)[]):string|number|object|null;
    set(value:NonNullable<string|number|object>,...keys: (string|undefined| null)[]):void;
    delete(...keys: (string|undefined | null)[]):void;
    destroy():void;
}
...
export class LocalCache implements ILocalCache{
    ...
    get(...keys: (string|undefined|null)[]): object | string | number | null {
    ...
    private createKey(...keys: (string|undefined|null)[]): string {
        try{
            return keys.filter(x => x!=null && x!=undefined && typeof x === 'string' && x.length > 0).join(":");
        }
        catch(err){
